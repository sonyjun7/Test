...
<body>
    ...
    <div id="hNotice">
        ...
        <?php
            //SPOT 테이블 출력
            $sql1 = mq("select spot_idx, structure_nm, state, city, town, road_nm_add from ARVR.SPOT where spot_idx='".$spotidx."'");
        ...
        ?>
        <div><a href="./toursearch_select.php"><button class="btn btn-primary">검색화면으로</button></a></div>
        ...
        <?php
            ...
            $sce_query = mq("SELECT * FROM scenario_info WHERE id='".$session."' ORDER BY idx desc");
            $sce_arr = $sce_query -> fetch_array();
                ...
                if($session != ""){
                    ...
                    for($i=1; $i<=5; $i++){
                        ...
                        $sss = mq("select * from scenario_info where id='".$session."' and idx='".$i."' order by idx");
                        ...
                        ?>
                            ...
                            <a class="ggg" href="./scenario_input.php?idx=<?php echo $sce_btn_cnt; ?>&spot_idx=<?php echo $sql_head['spot_idx']; ?>&state=<?php echo $sql_head['state']; ?>&structure=<?php echo $sql_head['structure_nm']; ?>&city=<?php echo $sql_head['city']; ?>"><button class="btn btn-success sce_btn_cls"><?php echo $sce_btn_cnt ?>번 째 시나리오 담기</button></a>
        <?php
        ...
        ?>
                    <a href="../member/mysce_info_select.php"><button class="btn btn-warning sce_btn_cls">담긴 시나리오 보기</button></a>
        <?php
                }
    ...
    <?php
        ...
        FROM ARVR.TEXTBOOK tbk INNER JOIN ARVR.TB_SPOT tbs ON tbk.tb_idx= tbs.tb_idx 
        AND tbs.spot_idx='".$spotidx."' AND tbk.subject LIKE CONCAT ('".$subject."', '%') AND tbk.year LIKE CONCAT('%', '".$year."' ,'%') INNER JOIN ARVR.TB_BGST tbg ON tbg.bpt_idx = tbs.bpt_idx AND tbg.tb_idx=tbs.tb_idx LEFT JOIN ARVR.BGST_MIDL bgm ON bgm.bpt_idx=tbs.bpt_idx GROUP BY tbk.tb_idx");

        while($sql_body1 = $sql2 -> fetch_array()){
    ?>

<div class="panel panel-primary p_body1">
...
<div class="panel panel-warning p_body2">
    ...
    <?php
        ...
        $sql3 = mq("SELECT DISTINCT tbk.tb_idx, lego.goal_txt FROM ARVR.TEXTBOOK tbk INNER JOIN ARVR.TB_SPOT tbs ON tbk.tb_idx= tbs.tb_idx AND tbs.spot_idx='".$spotidx."' AND tbk.subject LIKE CONCAT ('".$subject."', '%') AND tbk.year LIKE CONCAT('%', '".$year."' ,'%') INNER JOIN ARVR.TB_BGST tbg ON tbg.bpt_idx = tbs.bpt_idx AND tbg.tb_idx=tbs.tb_idx LEFT JOIN ARVR.BGST_MIDL bgm ON bgm.bpt_idx=tbs.bpt_idx INNER JOIN ARVR.LEARNING_GOAL lego ON lego.bpt_idx=tbs.bpt_idx");
    ...
    <div class="panel panel-default p_body3">
        ...
        <div class="panel-body p_table_body">
            <table class="table table-hover">
                ...
                <?php
                    $sql4 = mq("SELECT ATS.spot, ATS.road_nm_add FROM ARVR.ALL_TRVL_SPOT ATS INNER JOIN ARVR.SPOT_TRVL ST ON ATS.big_cd = ST.big_cd AND ST.spot_idx = '".$spotidx."'");
