<?php
    ...
    // }
    if(!empty($idx) && empty($_COOKIE['el_sce_' . $idx])){
        $cook = 'update scenario_post set hit= hit + 1 where post_idx=' . $idx;
        ...
        if(empty($elscehit)){
            ...
            setcookie('el_sce_' . $idx, TRUE, time() + (60 * 60 * 24), '/');
        }
    }
...
<?php
        ...
        $ttt = mq("select post_title from scenario_post where id='".$sceid."'");
    ...
    <div id="sce_area">
        ...
        <?php
            $scemq = mq("select * from scenario_info where id='".$sceid."' order by idx asc");
        ...
        ?>
                ...
                <?php
                    ...
                    $ifmq = mq("select * from scenario_info where id='".$sceid."' and idx='".$scebtn['idx']."'");
                ...
                ?>
            <form action="./courseRead.php" method="post">
                <button class="btn btn-primary btn_wa"><?php echo $scebtn['idx'] ?>번 코스 관람</button>
    ...
    <div id="cosebtn_area">
        <a href="./scenario_view_notice.php"><button class="btn btn-primary">시나리오 목록</button></a>
    </div>
