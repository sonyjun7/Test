...
<div id="all_graph_div">
    ...
    <?php
        $mq1 = mq("SELECT *,COUNT(state) FROM most_state WHERE id='".$session."' GROUP BY id, state ORDER BY COUNT(state) desc limit 5");
        ...
        // 장소 검색 횟수 확인(날짜별로 구본)
        $sql1 = mq("SELECT *,COUNT(date) FROM most_state WHERE id='".$session."' GROUP BY id, date ORDER BY date desc limit 5");
    ...
    <script>
        ...
        function drawChart1(){
            var data = google.visualization.arrayToDataTable(<?= json_encode($garr, JSON_UNESCAPED_UNICODE) ?>);
            console.log(data);
