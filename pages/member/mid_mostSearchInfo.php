...
<head>
    ...
    <title>중등 검색 기록 확인</title>
</head>
<body>

    <div class="panel panel-primary m_table">
        <div class="panel-heading">중등관광지 모든유저의 지역총합 검색수 확인</div>
        ...
        <?php
            ...
            // 장소 검색 횟수 확인(날짜별로 구본)
            $sql1 = mq("select state,COUNT(state) from most_state_mid group by state");
    ...
    <div class="panel panel-default m_table">
        <div class="panel-heading">중등관광지 모든유저의 장소총합 검색수 확인</div>
        <div class="panel-body p_table_body">
    ...
    <div class="panel panel-success m_table">
        <div class="panel-heading">중등 -- 날짜별 검색 지역 확인</div>
        <div class="panel-body p_table_body">
            <table class="table table-hover">
                <thead>
                    <tr>
        ...
        <?php
            ...
            // 장소 검색 횟수 확인(날짜별로 구본)
            $sql1 = mq("SELECT *,COUNT(state) FROM most_state_mid WHERE id='".$session."' GROUP BY id, state, date ORDER BY date desc");
            while($msearch = $sql1 -> fetch_array()){
        ?>
                <tbody>
                    <tr>
    ...
    <div class="panel panel-default m_table">
        <div class="panel-heading">중등 -- 최다 검색 지역 확인</div>
        <div class="panel-body p_table_body">
            <table class="table table-hover">
                <thead>
                    <tr>
        ...
        <?php
            ...
            $sql2 = mq("SELECT *,COUNT(state) FROM most_state_mid WHERE id='".$session."' GROUP BY id, state ORDER BY COUNT(state) desc");
    ...
    <div class="panel panel-warning m_table">
        <div class="panel-heading">중등 -- 지역별 최다 검색 장소 확인<br>(최다 검색 순)</div>
        <div class="panel-body p_table_body">
            <form id="str_form" method="get">
                <select name="structure_param" id="structure_param" class="form-control">
                    <?php
                        $sql3 = mq("select state_t, COUNT(*) from state_table left join (select * from most_state_mid where id='".$session."') ms on state_table.state_t = ms.state group by id, state_t order by COUNT(*) desc, state desc, state_t asc");
            ...
            <button id="structure_ok" class="btn btn-primary">장소 확인</button>
